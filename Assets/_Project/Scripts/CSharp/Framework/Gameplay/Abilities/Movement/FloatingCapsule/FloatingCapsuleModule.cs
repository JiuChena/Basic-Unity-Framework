...
namespace Framework.Gameplay.Abilities.Movement
{
    ...
    public sealed class FloatingCapsuleModule
    {
        ...
        /// <summary>计算不修改 Authoring 快照的编辑器预览形状。</summary>
        /// <param name="capsule">提供当前基础胶囊形状的组件。</param>
        /// <returns>按照当前浮动配置计算的预览胶囊形状。</returns>
        internal FloatingCapsuleShape GetPreviewShape(CapsuleCollider capsule)
        {
            if (capsule == null) return default;

            Vector3 localAxis = GetCapsuleLocalAxis(capsule.direction);
            float maximumClearance = Mathf.Max(0f, capsule.height - capsule.radius * 2f);
            float clearance = Mathf.Clamp(_bottomClearance, 0f, maximumClearance);
            if (!_enabled) clearance = 0f;
            return new FloatingCapsuleShape(
                capsule.center + localAxis * (clearance * 0.5f),
                capsule.height - clearance,
                capsule.radius,
                capsule.direction);
        }
