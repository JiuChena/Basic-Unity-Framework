...
namespace Framework.Gameplay.Abilities.Movement
{
    ...
    {
        // 误差低于该值且速度接近零时保持当前速度，消除物理步末端的微小往返。
        private const float RestErrorDeadZone = 0.003f;
        // 速度低于该值时视为悬浮系统已静止。
        private const float RestVelocityDeadZone = 0.05f;
        // 接地探测距离和悬浮弹簧配置。
            ...
            float fixedDeltaTime)
        {
            ...
            if (Mathf.Abs(heightError) <= RestErrorDeadZone
                && Mathf.Abs(normalVelocity) <= RestVelocityDeadZone)
                return velocity;

            // 最低使用临界阻尼，避免弹簧参数偏低时在目标高度两侧持续过冲。
            float criticalDamping = 2f * Mathf.Sqrt(Mathf.Max(0f, _settings.SpringStrength));
            float damping = Mathf.Max(_settings.SpringDamping, criticalDamping);
            float springAcceleration = heightError * _settings.SpringStrength;
            float dampingAcceleration = -normalVelocity * damping;
